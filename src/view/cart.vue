<template>
    <div class="cart">
        ...
        <div class="container">
            <div class="cart">
                ...
                <div class="cart-item">
                    ...
                    <li  v-for="(allcart,index) in cartList" :key="index">
                        <div class="cart-tab-1">
                            <div class="cart-item-check">
                            <a href="javascipt:;" class="checkbox-btn item-check-btn" v-bind:class="{'checked':allcart.checked=='1'}" @click="editCart('checked',allcart)">
                        ...
                        <div class="cart-item-pic">
                            <img :src="allcart.productImage">
                        </div>
                        <div class="cart-item-title">
                            <div class="item-name">{{allcart.productName}}</div>
                        </div>
                        </div>
                        <div class="cart-tab-2">
                        <div class="item-price">{{allcart.salePrice}}</div>
                        </div>
                        <div class="cart-tab-3">
                        <div class="item-quantity">
                            <div class="select-self select-self-open">
                            <div class="select-self-area">
                                <a class="input-sub" @click="editCart('minus',allcart)" href="javascript:;">-</a>
                                <span class="select-ipt">{{allcart.productNum}}</span>
                                <a class="input-add" href="javascript:;" @click="editCart('add',allcart)">+</a>
                            </div>
                            </div>
                        </div>
                        </div>
                        <div class="cart-tab-4">
                        <div class="item-price-total">{{allcart.salePrice*allcart.productNum}}</div>
                        </div>
                        <div class="cart-tab-5">
                        <div class="cart-item-opration">
                            <a href="javascript:;" class="item-edit-btn">
                            <svg class="icon icon-del" @click="delModalConfirm(allcart.productId)">
        ...
        <modal :md-show="delModalShow" @close="closeModalShow">
            <p slot="message">确定要删除该商品</p>
            <div slot="btnGroup">
                <a class="btn btn--m" href="javascript:;" @click="delModalShow = false">关闭</a>
                <a class="btn btn--m btn--red" href="javascript:;" @click="delItem">确定</a>
            </div>
...
<style>
  ...
  <script>
    ...
    export default{
        ...
        methods:{
            init(){
                axios.post('http://127.0.0.1:3000/users/cartlist').then((response)=>{
                    var res=response.data;
                    this.cartList=res.result;
                })
            },
            delModalConfirm(procuctId){
                this.delModalShow=true;
                this.productId=procuctId;
            },
            closeModalShow(){
                this.delModalShow=false;
            },
            delItem(productId){
                axios.post("http://127.0.0.1:3000/users/delcart",{productId:this.productId}).then((response)=>{
                    if(status==0){
                        this.delModalShow=false;
                        this.init();
                    }
            ...
            editCart(flag,target){
               
                if(flag=="add"){
                    target.productNum++;
                }else if(flag=="minus"){
                    if(target.productNum<=1){
                        target.productNum=1;
                    }else{
                        target.productNum--;
                    }
                    ...
                    target.checked=target.checked=='1'?'0':'1';
                };

                axios.post('http://127.0.0.1:3000/users/cartEdit',{
                    "productId":target.productId,
                    "checked":target.checked,
                    "productNum":target.productNum
                },(response)=>{
                    var res=response.data;
